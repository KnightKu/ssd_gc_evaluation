...
p["figure.figsize"] = (14.15, 20) # 两张A4纸大小，竖向页面
...
class my_draw:
    ...
    def generate_table_bandwidth(self, log):
        pl.xlabel('Time (Mins, Total 4*3 hour)')
        pl.ylabel('Bandwidth (MB/s)')
        self.log = log
        with open(self.log) as fd:
            for line in fd.readlines():
                if not len(line):
                ...
                if len(pair) != 2:
                    continue
                self.x.append(int (float(pair[0]) * 2.4))
                if pair[1].endswith('KB'):
                    self.y.append(float(pair[1][:-2]) / 1024)
                else:
                    self.y.append(float(pair[1][:-2]))

    def save(self):
        pl.savefig(os.path.basename(self.log))
...
draw = my_draw(1)
draw.generate_table_bandwidth('sdb_logs')
draw.draw(1)
draw.save()
